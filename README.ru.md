...
|---|---|
| **Инструменты** | Слева вставляешь, справа результат. Минифицированный JSON форматируется с сохранением порядка ключей; столбик id склеивается через запятую; unix-время превращается в дату, дата — в unix-время. Копирование одной кнопкой |
...
Обновить установленную сборку: `make update` подтягивает текущую ветку,
пересобирает, заменяет `/Applications/Cyclop.app` и перезапускает его.
